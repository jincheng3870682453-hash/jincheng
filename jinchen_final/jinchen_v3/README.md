...
│   ├── 05_github_scout.py      # 🔭 GitHub 侦察兵演示
│   └── dataset/                 # 标注数据集
│       └── sample_dataset.json
├── config/                       # 配置模板
...
```python
from Toolkit.work import RuleEngine, check
...
# 方式1：模块级快捷函数
report = check(code)
print(report.passed, report.violations)

# 方式2：RuleEngine 实例
engine = RuleEngine()
report = engine.check(code)
print(report.metrics)  # 复杂度、函数数等
```
...
### 1. 当前只做 Python，其他语言暂不支持

V3.2 做了一个**战略性的减法**：砍掉了 Java/Kotlin/TypeScript/Swift 的正则检测
（准确率低、维护成本高、正则骗不了人），**专注把 Python AST 规则磨到极致**。

| 语言 | 当前状态 | 说明 |
|------|---------|------|
| **Python** | ✅ 真 AST（13 条深度规则） | 唯一认真做的语言 |
| Java / Kotlin / TS / Swift | ❌ 已移除 | 等 `tree-sitter` 方案成熟再说 |

**结论**：这个工具集目前是"Python 治理专家"，不是"全栈治理专家"。
如果你的项目不是 Python，请等后续版本。
...
### 5. GitHub 侦察兵（github_scout）需要网络

`github_scout.py` 通过 `raw.githubusercontent.com` 拉取文件，**离线环境无法使用**。
拉取有 0.3 秒礼貌延迟，大量文件时较慢。结果会缓存在 `.scout_cache/` 目录。
...
| "我们写 Java/Swift" | 当前不支持 | 🔴 V3.2 砍掉了，等 tree-sitter |
