...
职责：
  ...
  - 违规时反馈原因让模型改（不是死循环重试）
  - 记录反馈飞轮数据
  - L1.5 项目感知：从对话历史里捞 GitHub 链接 + 文件结构

设计思路（借鉴 Skill Router 分层架构）：
  L1   流量过滤：清洗输入、拦截无效请求
  L1.5 项目感知：从对话/README 里捞 GitHub 链接 + 文件结构
  L2   规则匹配：关键词快速命中（零成本）
...
# ── 项目侦察兵（L1.5 层）─────────────────────────────────
try:
    from .github_scout import GithubScout, build_context_for_ai, quick_scan
    _SCOUT_AVAILABLE = True
except ImportError:
    _SCOUT_AVAILABLE = False
    log.warning("⚠️ github_scout 未加载，L1.5 项目感知不可用")
...
class WordGateway:
    ...
    # ── L1.5 项目感知层 ──────────────────────────────────
    def _project_awareness(self, user_input: str, conversation_history: list = None) -> dict:
        """
        L1.5 项目感知：从对话历史里捞 GitHub 链接 + 文件结构。
        ...
        流程：
          1. 扫描对话历史 → 找 GitHub 链接
          2. 扫描对话历史 → 找目录树 / 文件路径
          3. 拼出 raw URL → 拉取文件内容
          4. 生成给 AI 的"项目地图"prompt 片段
          5. 搜不到 → 返回反问话术，让 AI 问用户
        """
        if not _SCOUT_AVAILABLE:
            return {"has_project": False, "context": "", "scout": None}
        
        # 构建对话历史（把当前输入也算进去）
        history = list(conversation_history or [])
        if user_input and not any(h.get("content") == user_input for h in history):
        ...
        for msg in history:
            ...
            if m:
                repo_url = f"https://{m.group(0)}"
                break
        
        if not repo_url:
            return {"has_project": False, "context": "", "scout": None,
                    "followup": "我没有在你的对话里找到 GitHub 仓库链接。能告诉我你的项目地址吗？"}
        
        # 2. 创建 Scout 并解析
        try:
            scout = GithubScout(repo_url)
            scout.parse_multiple(history)
        except Exception as e:
            return {"has_project": False, "context": "", "scout": None,
                    "followup": f"解析仓库链接失败: {e}"}
        
        if not scout.files:
            # 搜不到文件 → 返回反问话术
            return {"has_project": True, "context": "", "scout": scout,
                    "followup": scout.build_followup_question()}
        
        # 3. 拉取文件
        try:
            result = scout.fetch_all()
            log.info(f"📂 项目感知: {result.fetched}/{result.total_files} 文件已拉取")
        except Exception as e:
            log.warning(f"⚠️ 拉取文件失败: {e}")
        
        # 4. 生成上下文
        context = scout.build_system_context()
        
        # 5. 智能匹配当前问题相关的文件
        relevant = scout.find(user_input[:50])
        if relevant:
            rel_ctx = f"\n\n📌 本次可能相关的文件:\n"
    ...
    def handle(self, user_input: str, conversation_id: str = "default") -> dict:
        ...
        # L1.5 项目感知（金呈核心思路：让 AI 看见项目结构）
        project_info = self._project_awareness(text, conversation_history=None)
        if project_info.get("followup"):
            log.info(f"📂 项目感知: 需要反问用户")
            # 没有项目上下文，正常走流程（不阻断）
        ...
        # 注入项目感知上下文（L1.5 的核心输出）
        if project_info.get("context"):
            context_parts.append(project_info["context"])
            log.info(f"📂 项目上下文已注入 ({len(project_info['files'])} 个文件)")
