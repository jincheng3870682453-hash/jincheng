...
try:
    ...
    # 1. 解析 GitHub 链接
    scout = gs.GithubScout("https://github.com/jincheng3870682453-hash/jinchen")
    check("  URL 解析: user", scout.repo.user == "jincheng3870682453-hash")
    check("  URL 解析: repo", scout.repo.repo == "jinchen")
    check("  URL 解析: branch", scout.repo.branch == "main")
    check("  raw_base 正确", "raw.githubusercontent.com" in scout.repo.raw_base)
...
└── verify.py
"""
    files = scout.parse_structure(tree_text)
    check("  目录树解析 > 0 文件", len(files) > 0, f"got {len(files)}")
    check("  提取 gateway.py", "Toolkit/gateway.py" in files)
    check("  提取 work.py", "Toolkit/work.py" in files)
    check("  提取 __init__.py", "Toolkit/__init__.py" in files)
    check("  提取 README.md", "README.md" in files)
    check("  提取 config_template.json", "config/config_template.json" in files)

    # 3. URL 拼接正确
    node = scout.files.get("Toolkit/gateway.py")
    if node:
        check("  URL 拼接正确",
              node.url == "https://raw.githubusercontent.com/jincheng3870682453-hash/jinchen/main/Toolkit/gateway.py")
    else:
        check("  URL 拼接正确", False, "node not found")

    # 4. 搜索功能
    results = scout.find("gateway")
    check("  搜索 'gateway'", len(results) > 0, f"got {results}")
    results2 = scout.find_by_ext("py")
    check("  按扩展名搜索 .py", len(results2) >= 3, f"got {len(results2)}")

    # 5. 搜不到时反问
    question = scout.ask_user("xxx_nonexist.py")
    check("  搜不到反问含确认提示", "哪个文件夹" in question or "文件名" in question)
    check("  搜不到反问不含路径", "xxx_nonexist" not in question.replace("xxx_nonexist.py", ""))

    # 6. 系统上下文
    ctx = scout.build_system_context()
    check("  系统上下文含仓库名", "jinchen" in ctx)
    check("  系统上下文含规则", "不要编造" in ctx or "不要" in ctx)

    # 7. 真实 README 解析（不报错）
    readme = Path("README.md")
    if readme.exists():
        real_files = scout.parse_structure(readme.read_text(encoding="utf-8"))
        check("  README 解析不报错", len(real_files) > 0, f"got {len(real_files)}")
        # 确认核心文件被提取
        has_py = any(f.endswith('.py') for f in real_files)
        check("  README 中提取到 .py 文件", has_py)
        has_skill = any(f.endswith('.skill') for f in real_files)
        check("  README 中提取到 .skill 文件", has_skill)
        # 确认没有误匹配中文标题
        bad = [f for f in real_files if '代码守门' in f or '快照回滚' in f]
        check("  无中文标题误匹配", len(bad) == 0, f"误匹配: {bad[:3]}")

    # 8. 序列化/反序列化
    scout.save("scout_test.json")
    scout2 = gs.GithubScout()
    scout2.load("scout_test.json")
    check("  保存/加载 JSON", len(scout2.files) == len(scout.files))
    Path("scout_test.json").unlink(missing_ok=True)

    # 9. 统计
    s = scout.stats()
    check("  stats 有数据", s['total'] > 0)
    check("  has_any() True", scout.has_any())

except Exception as e:
    import traceback
    traceback.print_exc()
    check("  GithubScout 模块可导入", False, str(e))
